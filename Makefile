...
DOCKER_TAG?=jaytwo_http_authentication_openidconnect
...
build: restore
	dotnet build ./jaytwo.Http.Authentication.OpenIdConnect.sln
...
unit-test: build
	...
	rm -rf out/coverage
	cd ./test/jaytwo.Http.Authentication.OpenIdConnect.Tests; \
		dotnet test \
		--results-directory ../../out/testResults \
		--logger "trx;LogFileName=jaytwo.Http.Authentication.OpenIdConnect.Tests.trx"
	reportgenerator \
...
pack:
	rm -rf out/packed
	cd ./src/jaytwo.Http.Authentication.OpenIdConnect; \
